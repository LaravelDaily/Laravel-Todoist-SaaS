        ...
        @foreach($plans as $plan)
            <div class="col">
                ...
                @if ($currentPlan && $plan->stripe_plan_id == $currentPlan->stripe_plan)
                    ...
                    @endif
                @else
                    <a href="#" class="btn btn-primary button-plan"
                       data-plan-id="{{ $plan->id }}"
                       data-plan-price="{{ config('saas.currency', '$') }}{{ number_format($plan->price / 100, 2) }}"
                       data-plan-title="{{ $plan->title }}">{{ trans('global.billing.choose_this_plan') }}</a>
                @endif
            </div>
        @endforeach
        </div>
